<?php
/**
 * XT_EHF Sites Notices
 *
 * @package XT_EHF Sites
 * @since 1.0.8
 */

if ( ! class_exists( 'XT_EHF_Notices' ) ) :
	...
	class XT_EHF_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_xt-ehf-notices', array( $this, 'dismiss' ) );
		...
		function enqueue_scripts() {
			wp_register_script( 'xt-ehf-sites-notices', XT_EHF_URL . 'assets/js/xt-ehf-sites-notices.js', array( 'jquery' ), XT_EHF_VER, true );
		}
		...
		function show_notices() {
			...
			foreach ( self::$notices as $key => $notice ) {
				...
				$classes = array( 'xt-ehf-notice', 'notice' );
				...
				// Notice ID.
				$notice_id    = 'xt-ehf-sites-notices-id-' . $notice['id'];
				$notice['id'] = $notice_id;
				if ( ! isset( $notice['id'] ) ) {
					$notice_id    = 'xt-ehf-sites-notices-id-' . $notice['id'];
					$notice['id'] = $notice_id;
		...
		public static function markup( $notice = array() ) {
			...
			wp_enqueue_script( 'xt-ehf-sites-notices' );
